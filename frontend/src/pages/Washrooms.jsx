...
export default function Washrooms() {
  ...
  const handleBook = (blockName) => {
    fetch('http://localhost:5000/api/washrooms/book', {
      method: 'POST',
  ...
  return (
    <div className="glass-panel" style={{ padding: '40px', maxWidth: '850px', margin: '0 auto' }}>
      <h2 style={{ fontSize: '2rem' }}>Washroom Live Status & Booking</h2>
      ...
      <div style={{ display: 'flex', gap: '15px', marginBottom: '30px', borderBottom: '2px solid #e2e8f0', paddingBottom: '25px' }}>
         <button 
            ...
            <Users size={24} /> Women's Washrooms
         </button>
         <button 
            ...
            <UserRound size={24} /> Men's Washrooms
         </button>
      </div>
      ...
      <div style={{ display: 'grid', gap: '25px' }}>
        ...
        {Object.keys(washrooms.women).length === 0 && <p style={{ color: '#64748b', textAlign: 'center', fontSize: '1.2rem' }}>Loading live sensor data...</p>}
      </div>
    </div>
  );
}
